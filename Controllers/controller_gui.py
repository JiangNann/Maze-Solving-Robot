...
#                    print("turning left for blind range")
    #                   1. turn left for sensor readings
                    ...
                    if updated_cells:
                        ...
                        #       update android
                        self._update_android()
                    # check for images when it happens to turn for blind range cells
                    if self.is_arrow_scan:
                        # to inform that no need to check again under normal conditions
                        front_and_left_image_detection = False
                        self._check_obstacle_image_right()
                        obstacles_right_2cells_away = self.is_image_possible_right_2cells_away()
                        if obstacles_right_2cells_away != False:
                            self.check_obstacles_for_image(obstacles_right_2cells_away)
                        
                        obstacles_front = self.is_image_possible_front(front_free=False,right_free=True)
                        obstacles_left = self.is_image_possible_left(front_free=False,right_free=True)
                        obstacles_back = False
                        # if front has an obstacle, try to check left & back by turning left too
                        if obstacles_front != False:
                            self._robot.turn_robot(self._sender, LEFT)
                            self._turn_head(self._facing, LEFT)
                            self._update_android()
                #            print('Obstacle(s) detected')
                #            print(obstacles)
                            self.check_obstacles_for_image(obstacles_front)
                            # if left has obstacle, then both left and back will be checked
                            if obstacles_left != False:
                                self._robot.turn_robot(self._sender, LEFT)
                                self._turn_head(self._facing, LEFT)
                                self._update_android()
                                self.check_obstacles_for_image(obstacles_left)
                                
                                if obstacles_back != False:
                                    self._robot.turn_robot(self._sender, LEFT)
                                    self._turn_head(self._facing, LEFT)
                                    self._update_android()
                                    
                                    self.check_obstacles_for_image(obstacles_back)
                                    
                                    # front, left, back all have obstacles, turn left to face the original direction after checking back
                                    self._robot.turn_robot(self._sender, RIGHT)
                                    self._turn_head(self._facing, RIGHT)
                                    
                                    # only front and left have obstacles, turn backward afte checking left
#                                else:
                                self._robot.turn_robot(self._sender, RIGHT)
                                self._robot.turn_robot(self._sender, RIGHT)
                             ...
                             # front has obstacles but left doesnt, turn right afteR checking front
                            else:
                                self._robot.turn_robot(self._sender, RIGHT)
                                self._turn_head(self._facing, RIGHT)
                                self._update_android()
                                
                                # check for obstacles at back
                                if obstacles_back != False:
                                    self._robot.turn_robot(self._sender, RIGHT)
                                    self._turn_head(self._facing, RIGHT)
                                    self._update_android()
                                    ...
                                    self._turn_head(self._facing, LEFT)
                                    self._update_android()
                        # front has no obstacle
                        else:
                             #front no obstacle but back has obstacle
                             if obstacles_back != False:
                                 self._robot.turn_robot(self._sender, RIGHT)
                                 self._turn_head(self._facing, RIGHT)
                                 self._update_android()
                                 ...
                                 self.check_obstacles_for_image(obstacles_back)
                                 # if only back & left have obstacles
                                 if obstacles_left != False:
                                     self._robot.turn_robot(self._sender, RIGHT)
                                     self._turn_head(self._facing, RIGHT)
                                     self._update_android()
                                     ...
                                     self.check_obstacles_for_image(obstacles_left)
                                     # turn back to original position after checking left
                                     self._robot.turn_robot(self._sender, LEFT)
                                     self._turn_head(self._facing, LEFT)
#                                     self._update_android()
                                     
#                                 else:
                                 # only back has obstacle, turn back after checking back
                                 self._robot.turn_robot(self._sender, LEFT)
                                 self._turn_head(self._facing, LEFT)
                                 self._update_android()
                             else:
                                 # only left has obstacle
                                 if obstacles_left != False:
                                     ...
                                     self._robot.turn_robot(self._sender, BB)
                                     self._turn_head(self._facing, BACKWARD)
                                     self._update_android()
...
#                             if obstacles_left != False:
                        ...
                        # robot only checked front, and is now back to original position
#                        if obstacles_left == False:
#                            if obstacles_back != False:
#                                self._robot.turn_robot(self._sender, RIGHT)
#                                self._turn_head(self._facing, RIGHT)
...
#                        obstacles_left = self.is_image_possible_left()
#                        if obstacles_left != False:
...
#                        front_and_left_image_detection = False
                ...
                # check for images under normal conditions
                self._check_obstacle_image_right()
                obstacles_right_2cells_away = self.is_image_possible_right_2cells_away()
                if obstacles_right_2cells_away != False:
                    self.check_obstacles_for_image(obstacles_right_2cells_away)
                                        
                obstacles_front = self.is_image_possible_front(front_free=front_free,right_free=right_free)
                obstacles_left = self.is_image_possible_left(front_free=front_free,right_free=right_free)
                obstacles_back = self.is_image_possible_back()
...
#                        self._check_obstacle_image_front()
#                        self._check_obstacle_image_left()
#                        self._check_obstacle_image_back()
...
#                print("end of the while loop")
        ...
        while not explore_complete:
            try:
                ...
                for move in moves:
                    ...
                    updated_cells1 = self._robot.mark_robot_standing()
    ...
    #                                 else:
                    ...
                    if updated_cells1 or updated_cells2:
                        #       update robot cells in UI as non-obstacle
        ...
        if self._robot.center == START:
#            print("obstacle facing dic:")
#            print(self._robot.obstacle_image_dic)
...
#        print(self._robot.facing)
    ...
    def check_free_front(self):
        ...
        return True
    def left_image_detection_worthy(self,y,x):
        return True
        if self._robot.facing == EAST:
            if y<=7:
                return True
...
#        return False
    ...
    def is_image_possible_left(self,front_free=False,right_free=True):
        #         N  N
        #    W 0, 1, 2
        #    W 3, 4, 5 E
        #      6, 7, 8 E
        ...
        #         y, x = get_matrix_coords(self._robot.center)
        y,x=get_matrix_coords(self._robot.center)
        # do not turn left for image if right is not free and front is not free since it will always turn left the next step
        if not right_free and not front_free:
            return False
...
#        1,2
        ...
        # if no obstacle 2 cells away, check 3 cells away
        if obstacles == [(0,0,0),(0,0,0)]:
#            print("here gonna check 3 cells away")
            obstacles = self.is_image_possible_left_3cells_away(facing=facing,y=y,x=x,front_free=front_free,right_free=right_free)
            if obstacles != False:
                return obstacles
            return False
            
        # if next step is to move to front
        if not right_free and front_free:
#         if middle cell needs to be seen, return True
            if obstacles[0]!=(0,0,0):
                return obstacles
            return False
        # if have at least 1 obstacle need to be seen & next movement is to right
        return obstacles

    def is_image_possible_left_3cells_away(self,facing,y,x,front_free=False,right_free=True):
#        print("inside is_image_possible_left_3cells_away func")
            #         N  N
            #    W 0, 1, 2
            #    W 3, 4, 5 E
            #      6, 7, 8 E
...
#            print("here checking 3 cells away left when facing east")
#        1,2
            y1= y+4
            x1 = x-1
            if y1 > 19:
...
#            print("aaaaaaa")
            return False
#            if right not free and front is free, will move to front for the next step
        if not right_free and front_free:
    #         if left cell needs to be seen, return True
            if obstacles[0]!=(0,0,0):
                return obstacles
            return False
#        if right is free
    ...
    def is_image_possible_back(self):
    #         N  N
        #    W 0, 1, 2
        #    W 3, 4, 5 E
        #      6, 7, 8 E
...
#        1,2
        ...
        #              either one is obstacle & that face is not scanned:
        facing = (self._robot.facing + BACKWARD) % 4
        obstacle1 = 0,0,0
        obstacle2 = 0,0,0
        obstacles = [(0,0,0),(0,0,0)]
        if (self._robot.discovered_map[y1][x1] == 1):
        ...
        if (self._robot.discovered_map[y2][x2] == 1):
            if (facing not in self._robot.obstacle_image_dic[y2, x2]) and (-1 not in self._robot.obstacle_image_dic[y2, x2]):
                obstacles[1] = y2,x2,facing
        if obstacles == [(0,0,0),(0,0,0)]:
            obstacles = self.is_image_possible_back_3cells_away(facing=facing,y=y,x=x)
            if obstacles != False:
                return obstacles
            return False
        return obstacles
    
    def is_image_possible_right_2cells_away(self):
            #         N  N
            #    W 0, 1, 2
            #    W 3, 4, 5 E
            #      6, 7, 8 E
...
#            return obstacles
            ...
            if obstacles == [(0,0,0),(0,0,0)]:
#                print("here gonna check 3 cells away")
#                obstacles = self.is_image_possible_front_3cells_away(facing=facing,y=y,x=x,front_free=front_free,right_free=right_free)
#                if obstacles != False:
#                    return obstacles
                return False
    ...
    def is_image_possible_front(self,front_free,right_free):
        #         N  N
        #    W 0, 1, 2
        #    W 3, 4, 5 E
        #      6, 7, 8 E
...
#        if right is free
        return obstacles
#        if front free => should return obstacles
#         if front not free => should not return
...
#        return False
    def is_image_possible_back_3cells_away(self,facing,y,x):
#        print("inside is_image_possible_back_3cells_away func")
            #         N  N
            #    W 0, 1, 2
            #    W 3, 4, 5 E
            #      6, 7, 8 E
...
#        facing = self._robot.facing
        ...
        if (self._robot.discovered_map[y1][x1] == 1):
            if (facing not in self._robot.obstacle_image_dic[y1, x1]) and (-1 not in self._robot.obstacle_image_dic[y1, x1]):
                obstacles[0] = y1,x1,facing
#                print("here there is obstacle 3 cells away cell 1")
#                print(obstacles[0])
#                return obstacle1
        if (self._robot.discovered_map[y2][x2] == 1):
            if (facing not in self._robot.obstacle_image_dic[y2, x2]) and (-1 not in self._robot.obstacle_image_dic[y2, x2]):
                obstacles[1] = y2,x2,facing
#                print("here there is obstacle 3 cells away cell 2")
#                print(obstacles[1])
#                return obstacle2
        if (self._robot.discovered_map[y3][x3] == 1):
            if (facing not in self._robot.obstacle_image_dic[y3, x3]) and (-1 not in self._robot.obstacle_image_dic[y3, x3]):
                obstacles[2] = y3,x3,facing
#                print("here there is obstacle 3 cells away cell 3")
#                print(obstacles[2])
#                return obstacle3
        if obstacles == [(0,0,0),(0,0,0),(0,0,0)]:
            return False
    ...
    def is_image_possible_front_3cells_away(self,facing,y,x,front_free,right_free):
...
#        print("Asking RPI for single image from 1/2 cell away")
    ...
    def check_obstacles_for_image(self,obstacles):
        #        ask rpi to detect
    #    y, x, facing = obstacle
#        print("Asking RPI for images from 2/3 cells away")
...
#            print("candidate obstacles:")
#            print(obstacles)
#            single_obstacle = False
#           no obstacle at the index, use its neighbour obstacle
            if obstacles[obstacle_index] == (0,0,0):
#                print("returned index has no obstacle!! randomly assign image to its neighbour obstacle")
                for (y,x,facing) in obstacles:
                    if (y,x,facing) != (0,0,0):
                        self._sender.send_android('ID' + ',' + image_ID + ',' + str(x) + ',' + str(y))
                        self._robot._mark_neigbourhood_obsolete(y, x)
            else:
#                print("returned index has obstacle")
...
#        print("end of check image front")
    ...
    def _check_obstacle_image_back(self):
#        print("start of check image back")
#            ''' front '''
        obstacles = self.is_image_possible_back()
        if obstacles != False:
            self._robot.turn_robot(self._sender, RIGHT)
            self._turn_head(self._facing, RIGHT)
            self._update_android()
...
#           ''' left '''
        obstacles = self.is_image_possible_left()
        if obstacles != False:
#            print("should turn for left image")
            self._robot.turn_robot(self._sender, BACKWARD)
            self._turn_head(self._facing, BACKWARD)
            self._update_android()
...
#                self.check_obstacles_for_image(useful_obstacles_left[0],useful_obstacles_left[1])
    ...
    def is_image_possible_right(self):
        ...
        #      E  E
        y, x = get_matrix_coords(self._robot.center)
#        print("checking image right facing:")
#        print(self._robot.facing)
        if self._robot.facing == NORTH:
#        5,8
            y1= y
...
#            return y1,x1,facing
        if (self._robot.discovered_map[y1][x1] == 1):
            if (facing not in self._robot.obstacle_image_dic[y1, x1]) and (-1 not in self._robot.obstacle_image_dic[y1, x1]):
#                print("obstacle detected for right, checking image")
