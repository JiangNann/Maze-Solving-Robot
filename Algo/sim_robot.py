...
class Robot:
    ...
    def __init__(self, exploration_status, facing, discovered_map, real_map):
        ...
        self.sensors = [
            ...
            {"mount_loc": NWS, "facing": WEST, "range": 5, "blind_spot": 2}
        ]
...
#        or (self.center in E_LIMITS[WEST] and self.facing == NORTH):
    ...
    def get_completion_count(self):
        """
        Calculate how many of the cells the robot has explored.
    ...
    def is_complete(self, explore_limit, start_time, time_limit):
        ...
        """
        return self.get_completion_count() >= explore_limit or float(time() - start_time >= time_limit)
    ...
    def check_free(self, direction):
        ...
        try:
            ...
            # w.r.t arena i.e. the direction is to the north of arena
                y, x = get_matrix_coords(robot_cells[0])
                y += 1
                print('Cell to check : {}'.format((x, y, x+1, y, x+2, y)))
                # if it's to check if the north direction is free, return 1 row above the robot's upper most row
                # x, y,  x+1, y,  x+2, y
                #   0,      1,       2
                #   3,      4,       5
                #   6,      7,       8
                ...
                # if none of the cells in the upper row is obstacle, return is_free
    ...
    def get_sensor_readings(self, is_arrow_scan = False):
        ...
        for sensor in sensors:
            ...
            elif sensor_index(sensor) == 0 and reading in blind_range:
